...
#
# To support build number generated by a CI server, the specification
# of war file name/location is a little twisted. You need to pass:
#       * -w or $APP_WAR:         Eg ../target/scala-2.10/app_2.10-0.1
#       * -v or $CI_BUILD_NUMBER: Eg 456
#
# eb-deploy will combine those to create the actual war file: $APP_WAR.$CI_BUILD_NUMBER.war
...
OPTIONS:
-h      Show this message
-w      \$APP_WAR                    | war file location of the form ../target/scala-2.10/app_2.10-0.1
...
APP_WAR='../build/tweetmap'
CI_BUILD_NUMBER='1.0.0'
...
;;
p)
APP_EB_InstanceProfileName=$OPTARG
;;
a)
APP_EB_ApplicationName=$OPTARG
;;
e)
APP_EB_EnvironmentName=$OPTARG
;;
?)
usage
exit
...
# install boto (required by AWS CLI)
pip install boto

# Ensure war file exists
WAR=$APP_WAR.$CI_BUILD_NUMBER.war
if [ ! -f $WAR ]; then
echo "War file not found at $WAR. eb-deploy looks at (\$APP_WAR or -w).(\$CI_BUILD_NUMBER or -v).war"
usage
exit 1
fi
